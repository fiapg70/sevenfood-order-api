...
docker build -t rogeriofontes/sevenfood-order-api:v3 .
docker login
docker push rogeriofontes/sevenfood-order-api:v3

docker pull rogeriofontes/sevenfood-order-api:v3
docker run -p 9996:9996 sevenfood-order-api:v3
...
docker run -u zap -p 8080:8080 -v "$(pwd)/docs/zap_workdir:/zap/wrk" -i ghcr.io/zaproxy/zaproxy:stable zap-api-scan.py -t http://192.168.100.31:9996/api/v3/api-docs -f openapi -c zap-rules.conf -r zap_report_antes_op1.html
